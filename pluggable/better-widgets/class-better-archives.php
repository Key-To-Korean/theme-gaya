...
class Better_Archives extends WP_Widget {
	...
	public function __construct() {
		...
		$widget_ops = array(
			...
			'description' => __( 'A yearly, then monthly, archive of your site&#8217;s Posts.', 'gaya' ),
		);
		parent::__construct( 'better_archives', __( 'Better Archives', 'gaya' ), $widget_ops );
	...
	public function widget( $args, $instance ) {
		...
		$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? esc_html__( 'Archives', 'gaya' ) : $instance['title'], $instance, $this->id_base );
		...
		if ( ! empty( $title ) ) {
			...
			echo esc_html( $args['before_title'] . __( 'Better Archives', 'gaya' ) . $args['after_title'] );
		}
		...
		$months = wp_cache_get( 'gaya_better_archives_months', 'widget' );

		if ( false === $months ) {
			// $wpdb->posts is derived internally from the site's own table prefix, not user input.
			...
			wp_cache_set( 'gaya_better_archives_months', $months, 'widget', HOUR_IN_SECONDS );
		}

		if ( ! empty( $months ) ) :

			echo '<ul class="gaya-widget-list">';
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'widget_title' ) ); ?>"><?php esc_html_e( 'Title', 'gaya' ); ?>:
		...
		<!--		
		<p>
			<input class="checkbox" type="checkbox"<?php // checked( $instance[ 'dropdown' ] ); ?> id="<?php // echo $this->get_field_id( 'dropdown' ); ?>" name="<?php // echo $this->get_field_name( 'dropdown' ); ?>" /> <label for="<?php // echo $this->get_field_id( 'dropdown' ); ?>"><?php // _e( 'Display as dropdown', 'gaya' ); ?></label>
			<br/>
			<input class="checkbox" type="checkbox"<?php // checked( $instance[ 'count' ] ); ?> id="<?php // echo $this->get_field_id( 'count' ); ?>" name="<?php // echo $this->get_field_name( 'count' ); ?>" /> <label for="<?php // echo $this->get_field_id( 'count' ); ?>"><?php // _e( 'Show post counts', 'gaya' ); ?></label>
		</p>
		-->
		<?php
...
function gaya_comments_widget_registration() {
	unregister_widget( 'WP_Widget_Recent_Comments' );
	register_widget( 'gaya_recent_comments' );
}
add_action( 'widgets_init', 'gaya_comments_widget_registration' );
*/
