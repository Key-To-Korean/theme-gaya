...
function gaya_author_box() {
	...
	if ( isset( $post->post_author ) ) {
		...
		*/
		...
		/*
		Old version.
		// $author_details .= '<h3 class="author-title"><span>' . esc_html__( 'About ', 'gaya' );
		// if ( ! empty ( $display_name ) ) $author_details .= $display_name;				// If an author archive, just show the author name
		// else $author_details .= esc_html__( 'the Author', 'gaya' ); // If a regular page, show "About the Author"
		...
		*/
		...
		if ( ! is_author() ) { // Don't show the meta info on an author archive page.
			$author_details .= '<p class="author-links entry-meta"><span class="vcard">' . esc_html__( 'Read more posts by ', 'gaya' ) . '<a class="fn" href="' . esc_url( $user_posts ) . '">' . $display_name . '</a></span>';

			// Check if author has a website in their profile.
			if ( ! empty( $user_site ) ) {
				$author_details .= '<a class="author-site" href="' . esc_url( $user_site ) . '" target="_blank" rel="nofollow"><i class="fa fa-globe"></i><span class="screen-reader-text">' . esc_html__( 'Website', 'gaya' ) . '</span></a></p>';
			} else {
				$author_details .= '</p>';
			}
		}
		...
		$author_details .= '<p class="show-hide-author label btn"><i class="fa fa-minus-circle"></i> ' . esc_html__( 'Hide', 'gaya' ) . '</p>';
