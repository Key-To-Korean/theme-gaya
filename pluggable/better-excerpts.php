<?php
/**
 * Better Post Excerpts File
 *
 * @package gaya
 */

/**
 * Better Post Excerpts Function
 *
 * Based on Post Format, it trims the excerpt in various ways and returns various pieces of content
 *
 * @param [string] $text Post Text.
 * @param [string] $raw_excerpt Excerpt.
 */
function gaya_better_excerpts( $text, $raw_excerpt ) {
	...
	} elseif ( 'chat' === get_post_format() && ! $raw_excerpt ) {
		/**
		 * Post Format: Chat
		 *
...
add_filter( 'wp_trim_excerpt', 'gaya_better_excerpts', 5, 2 );

/**
 * Add Excerpts to Pages
 */
function gaya_add_excerpt_to_pages() {
	add_post_type_support( 'page', 'excerpt' );
}
add_action( 'init', 'gaya_add_excerpt_to_pages' );

/**
 * Customize ellipsis at end of excerpts.
 *
 * @param mixed $more Hyperlink.
 */
function gaya_excerpt_more( $more ) {
...
add_filter( 'excerpt_more', 'gaya_excerpt_more' );

/**
 * Filter excerpt length to 100 words.
 *
 * @param mixed $length Length of the excerpt.
 */
function gaya_excerpt_length( $length ) {
	return 40;
}
add_filter( 'excerpt_length', 'gaya_excerpt_length' );
 ...
 */
function gaya_fancy_excerpt() {
	global $post;
	if ( has_excerpt() ) :
		the_excerpt();
	elseif ( false !== strpos( $post->post_content, '<!--more-->' ) ) :
		the_content();
