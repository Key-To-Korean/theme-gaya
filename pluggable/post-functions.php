...
function gaya_homepage_limits( $limits ) {
	if ( is_home() ) {
			return 'LIMIT 0, 30';
	}
	return $limits;
}
add_filter( 'post_limits', 'gaya_homepage_limits' );
 */
 ...
 */
function gaya_modify_archive_title( $title ) {
	if ( is_page_template( 'archive-jetpack-portfolio.php' ) || is_page_template( 'archive-jetpack-testimonial.php' ) ) {
			return esc_html__( 'All ', 'gaya' ) . $title;
	} else {
			return $title;
	}
}
add_filter( 'get_the_archive_title', 'gaya_modify_archive_title', 10, 1 );
 ...
 */
function gaya_odd_even_post_classes( $classes ) {
...
add_filter( 'post_class', 'gaya_odd_even_post_classes' );
global $current_class;
$current_class = 'odd';
 ...
 * @return bool
 */
function gaya_show_reading_time() {
	$content = get_post_field( 'post_content', get_the_ID() );
	return in_array( get_post_type(), array( 'post' ), true ) && ! empty( $content ) && (bool) 1 === (bool) get_theme_mod( 'gaya_display_reading_time', 1 );
}

if ( ! function_exists( 'gaya_reading_time' ) ) :
	/**
 ...
 */
function gaya_validate_gravatar( $id_or_email ) {
...
if ( ! function_exists( 'gaya_post_nav' ) ) :
	/**
	...
	function gaya_post_nav() {
		...
		<nav class="navigation post-navigation clear" role="navigation">
			<h2 class="screen-reader-text"><?php esc_attr_e( 'Post navigation', 'gaya' ); ?></h2>
				...
				<div class="nav-links">
						...
						<div class="nav-previous">
								<a href="<?php echo esc_attr( get_permalink( $prev_id ) ); ?>" rel="prev">
										...
										<h3 class="meta-nav section-title" aria-hidden="true"><?php esc_attr_e( 'Previously', 'gaya' ); ?></h3>
										<span class="screen-reader-text"><?php esc_attr_e( 'Previous Post', 'gaya' ); ?></span>
						...
						<div class="nav-next">
								<a href="<?php echo esc_attr( get_permalink( $next_id ) ); ?>" rel="next">
										...
										<h3 class="meta-nav section-title" aria-hidden="true"><?php esc_attr_e( 'Next time', 'gaya' ); ?></h3>
										<span class="screen-reader-text"><?php esc_attr_e( 'Next Post', 'gaya' ); ?></span>
...
if ( ! function_exists( 'gaya_paging_nav' ) ) :
	/**
	...
	function gaya_paging_nav() {
		...
		$links = paginate_links(
			array(
				...
				'prev_text' => __( '<i class="fa fa-chevron-left"></i>', 'gaya' ),
				'next_text' => __( '<i class="fa fa-chevron-right"></i>', 'gaya' ),
				'type'      => 'list',
			)
		);
		...
		<nav class="navigation paging-navigation" role="navigation">
			<h1 class="screen-reader-text"><?php esc_attr_e( 'Posts navigation', 'gaya' ); ?></h1>
 ...
 * Full stop.
 */
function gaya_get_the_first_image_url( $size ) {
