 ...
 */
function gaya_using_amp_live_list_comments() {
	if ( ! gaya_is_amp() ) {
		return false;
	}
 ...
 */
function gaya_add_amp_live_list_pagination_attribute( $markup ) {
	...
	if ( is_home() && ! is_front_page() ) {
		...
		if ( is_day() ) {
			...
			<header class="page-header">
				<h1 class="page-title">
					<?php esc_html_e( 'Published on this day:', 'gaya' ); ?>
			...
			<header class="page-header">
				<h1 class="page-title">
					<?php esc_html_e( 'Published in this month:', 'gaya' ); ?>
			...
			<header class="page-header">
				<h1 class="page-title">
					<?php esc_html_e( 'Published this year:', 'gaya' ); ?>
					<span><?php echo $year; // WPCS: XSS OK. ?></span>
				</h1>
			</header>
			<?php
		}
		...
		<header class="page-header">
			...
			<?php
				...
				printf( esc_html__( 'Search Results for: %s', 'gaya' ), '<span>' . get_search_query() . '</span>' );
			?>
			</h1>
		</header><!-- .page-header -->
		<?php
		...
		<header class="page-header">
			...
			if ( ! empty( $first_name ) ) {
				...
				<h1 class="page-title">
					<?php
					/* translators: %s: search query. */
					printf( esc_html__( 'Author: %s', 'gaya' ), '<span>' . esc_attr( the_archive_title() ) . '</span>' );
					?>
				</h1>
				<?php
			}
...
function gaya_post_categories() {
	...
	if ( 'post' === get_post_type() ) {
		...
		$categories_list = get_the_category_list( esc_html__( ' &bull; ', 'gaya' ) );
		if ( $categories_list ) {
		...
		comments_popup_link(
			sprintf(
				wp_kses(
					...
					__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'gaya' ),
					array(
						'span' => array(
							'class' => array(),
						),
 ...
 */
function gaya_edit_post_link() {
	edit_post_link(
		sprintf(
			wp_kses(
				/* translators: %s: Name of current post. Only visible to screen readers */
				__( 'Edit <span class="screen-reader-text">%s</span>', 'gaya' ),
				array(
					'span' => array(
						'class' => array( 'btn' ),
					),
 ...
 */
function gaya_attachment_in( $post ) {
	if ( ! empty( $post->post_parent ) ) :
		$postlink = sprintf(
			/* translators: %s: original post where attachment was added. */
			esc_html_x( 'in %s', 'original post', 'gaya' ),
 ...
 */
function gaya_the_attachment_navigation() {
	?>
	<nav class="navigation post-navigation" role="navigation">
		<h2 class="screen-reader-text"><?php echo esc_html__( 'Post navigation', 'gaya' ); ?></h2>
		<div class="nav-links">
			<div class="nav-previous">
				<div class="post-navigation-sub">
					<?php echo esc_html__( 'Previous attachment:', 'gaya' ); ?>
				</div>
				<?php previous_image_link( false ); ?>
			</div><!-- .nav-previous -->
			<div class="nav-next">
				<div class="post-navigation-sub">
					<?php echo esc_html__( 'Next attachment:', 'gaya' ); ?>
				</div>
 ...
 */
function gaya_get_the_excerpt( $post_id = null ) {
	if ( empty( $post_id ) ) {
		$excerpt = '';
	} else {
 ...
 */
function gaya_archive_thumbnails() {
	/* Grab AN img URL to set as the background of the section */
	if ( has_post_thumbnail() ) {
		$img_url = get_the_post_thumbnail_url( get_the_ID(), 'full' );
	} elseif ( '' !== gaya_get_the_first_image_url( 'large-thumb' ) ) {
		$img_url = gaya_get_the_first_image_url( 'large-thumb' );
		...
		*/
		...
		$img_url = gaya_placeholder_image_url();
	}
	?>
 ...
 */
function gaya_placeholder_image_url() {
 ...
 */
function gaya_header_flash() {
	?>
	<!-- Top "News Flash" type section -->
	<div class="header-flash <?php echo esc_attr( $header_img_class ); ?>">
		<div class="header-contact">
			<?php $gaya_description = get_bloginfo( 'description', 'display' ); ?>
			<?php if ( $gaya_description || is_customize_preview() ) : ?>
				<p class="site-description"><?php echo $gaya_description; /* WPCS: xss ok. */ ?></p>
			<?php endif; ?>
