  ...
  <section id="featured-slider" class="featured-posts">
    ...
    while( $featured->have_posts() ) : $featured->the_post();
      ...
      <article <?php $img_url !== '' ? post_class( 'featured' ) : post_class( 'featured', 'placeholder' ); ?> id="post-<?php the_ID(); ?>" 
        <?php echo $img_url !== '' ? 'style="background-image: url(' . $img_url . ');"' : ''; ?>>
      ...
      <div class="entry-meta entry-meta-left">
          <?php 
          echo '<span class="entry-permalink"><a href="' . get_permalink() . '" title="' . __( 'Read the article ', 'wprig' ) . get_the_title() . '" rel="bookmark">Read Article <i class="fa fa-arrow-right"></i></a></span>'; 
          wprig_reading_time();
          ?>
      </div><!-- .entry-meta -->
