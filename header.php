				...
				<?php
					wp_nav_menu(
						array(
							...
							'link_after'     => '</span>' . gaya_get_svg( array( 'icon' => 'chain' ) ),
						)
					);
				?>
...
<div id="page" class="site">
	<a class="skip-link screen-reader-text" href="#primary"><?php esc_html_e( 'Skip to content', 'gaya' ); ?></a>
	...
	>
		...
		<header id="masthead" class="site-header">
					...
					<?php if ( is_front_page() && is_home() ) : ?>
						<h1 class="site-title">
							...
							<?php if ( $gaya_description || is_customize_preview() ) : ?>
								<span class="site-description"><?php echo $gaya_description; /* WPCS: xss ok. */ ?></span>
							<?php endif; ?>
						</h1>
					<?php else : ?>
						<p class="site-title">
			...
			<div class="position-right">
				...
				<button class="menu-toggle" aria-label="<?php esc_attr_e( 'Open menu', 'gaya' ); ?>" aria-controls="primary-menu" aria-expanded="false"
					<?php if ( gaya_is_amp() ) : ?>
						on="tap:AMP.setState( { siteNavigationMenu: { expanded: ! siteNavigationMenu.expanded } } )"
						[aria-expanded]="siteNavigationMenu.expanded ? 'true' : 'false'"
					<?php endif; ?>
				>
					<?php esc_html_e( 'Menu', 'gaya' ); ?>
				</button>
			...
			<nav id="site-navigation" class="main-navigation" aria-label="<?php esc_attr_e( 'Main menu', 'gaya' ); ?>"
				<?php if ( gaya_is_amp() ) : ?>
